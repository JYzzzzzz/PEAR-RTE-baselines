...
import os
import sys
from dataprocess.data_processor import token_decode, Char_Token_SpanConverter
...
def get_e2r(e2r_pred, token_len):
    ...
    从给定的注意力矩阵中提取实体-关系（主体-关系）和实体-实体交互。
    只提取右上角的三角形，所以应该输入原始注意力矩阵的转置来提取关系-实体（关系-客体）的交互。
    输入：矩阵，句子长度
    输出：
    """
    e2r = {}
...
def unirel_span_extractor(tokenizer,
    ...
    span_converter = Char_Token_SpanConverter(tokenizer=tokenizer, add_special_tokens=True)

    # Minus the [cls] and [sep] 
    token_len = dataset.max_length - 2
    # token_len = dataset.max_length

    state_dict = {"p": 0, "c": 0, "g": 0}   # p: predict, g: gold, c: correct
    e2e_state_dict = {"p": 0, "c": 0, "g": 0}
    e2e_tail_state_dict = {"p": 0, "c": 0, "g": 0}
    e2e_plain_state_dict = {"p": 0, "c": 0, "g": 0}
    h2r_state_dict = {"p": 0, "c": 0, "g": 0}
    t2r_state_dict = {"p": 0, "c": 0, "g": 0}
    idx2pred = dataset.data_processor.idx2pred
    extract_data = []     # prediction of all samples
    path = os.path.join(path, f"prediction_{dataset.mode}.json")
    ...
    for head_pred, tail_pred, span_pred in zip(head_preds, tail_preds, span_preds):
        ...
        for spo in spo_span_list:
            ...
            gold_spo_text.add((left_str, rel_str, right_str))
        # # for test. 发现span全部右移一格的bug。最后发现是标注时考虑了2遍[CLS]，导致右移了。
        # head_label = dataset[curr_data_idx]["head_label"]
        # tail_label = dataset[curr_data_idx]["tail_label"]
        # span_label = dataset[curr_data_idx]["span_label"]
        # s_h2r, s2s = get_e2r(head_label, token_len)   # 关系与主体起始，
        # s_t2r, _ = get_e2r(head_label.T, token_len)   # 关系与客体起始
        # e_h2r, e2e = get_e2r(tail_label, token_len)   # 关系与主体结束
        # e_t2r, _ = get_e2r(tail_label.T, token_len)   # 关系与客体结束
        # start2span, end2span = get_span_att(span_label, token_len)  #
        # print("text")
        # print(text)
        # print("input_ids")
        # print(input_ids)
        ...
        # 获取 prediction text
        triples_out = []
        pred_spo_text = set()   # prediction info of one sample
        s_h2r, s2s = get_e2r(head_pred, token_len)   # 关系与主体起始，
        s_t2r, _ = get_e2r(head_pred.T, token_len)   # 关系与客体起始
        e_h2r, e2e = get_e2r(tail_pred, token_len)   # 关系与主体结束
        e_t2r, _ = get_e2r(tail_pred.T, token_len)   # 关系与客体结束
        ...
        for l, r in e2e:
            ...
            for l_span in l_spans:
                for r_span in r_spans:
                    ...
                    subj_span_new = (l_span[0], l_span[1])    # 生成的token跨度
                    obj_span_new = (r_span[0], r_span[1])
                    for rel in common_rels:
                        subj_str = token_decode(tokenizer, input_ids[subj_span_new[0]:subj_span_new[1]+1])
                        obj_str = token_decode(tokenizer, input_ids[obj_span_new[0]:obj_span_new[1]+1])
                        pred_spo_text.add((subj_str, idx2pred[rel], obj_str))
                        # ^^^ 原先的案例记录，保留，用于统计

                        # jyz chg 2409. 记录案例，格式与原先不同
                        subj_char_span = span_converter.get_char_span(text, (subj_span_new[0], subj_span_new[1]+1))
                        obj_char_span = span_converter.get_char_span(text, (obj_span_new[0], obj_span_new[1] + 1))
                        rela_str = idx2pred[rel]
                        subj_str = text[subj_char_span[0]:subj_char_span[1]]
                        obj_str = text[obj_char_span[0]:obj_char_span[1]]
                        triple_out = {
                            "subject": subj_str,
                            "predicate": rela_str,
                            "object": obj_str,
