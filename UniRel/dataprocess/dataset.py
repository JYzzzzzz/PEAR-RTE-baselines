...
class UniRelSpanDataset(Dataset):
    ...
    def __getitem__(self, idx):   # 输入下标
        ...
        sep_idx = inputs["input_ids"].index(self.tokenizer.sep_token_id)
        input_ids = inputs["input_ids"] + self.pred_inputs["input_ids"]  # 连接 句子部分 与 关系部分
        input_ids[sep_idx] = self.tokenizer.pad_token_id

        attention_mask = inputs["attention_mask"] + [1] * num_rels
        attention_mask[sep_idx] = 0
        token_type_ids = inputs["token_type_ids"] + [1] * num_rels   # token_type_ids 关系部分为1


        return {
            "input_ids":
            torch.tensor(np.array(input_ids, dtype=np.int64), dtype=torch.long),
            "attention_mask":
            torch.tensor(np.array(attention_mask, dtype=np.int64), dtype=torch.long),
            "token_type_ids":
            torch.tensor(np.array(token_type_ids, dtype=np.int64), dtype=torch.long),
            "token_len_batch":
            torch.tensor(token_len, dtype=torch.long),
