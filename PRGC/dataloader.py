...
from dataloader_utils import read_examples, read_examples_cmim, convert_examples_to_features


def tokenizer_init(tokenizer_file_path):   # jyz chg
    # tokenizer = BertTokenizer(vocab_file=tokenizer_file_path,
    #                                do_lower_case=False)
    tokenizer = BertTokenizer(vocab_file=tokenizer_file_path,
                                   do_lower_case=True)  # jyz chg
    return tokenizer
...
class CustomDataLoader(object):
    def __init__(self, params):
        ...
        tokenizer_file_path = os.path.join(params.bert_model_dir, 'vocab.txt')
        self.tokenizer = tokenizer_init(tokenizer_file_path)   # jyz chg
    ...
    def get_features(self, data_sign, ex_params):
        ...
        if os.path.exists(cache_path) and self.data_cache:
            ...
            if data_sign in ("train", "val", "test", "pseudo", 'EPO', 'SEO', 'SOO', 'Normal', '1', '2', '3', '4', '5'):
                # !!!!!!!!!!!!!! read examples, jyz chg
                # examples = read_examples(self.data_dir, data_sign=data_sign, rel2idx=rel2idx)
                examples = read_examples_cmim(self.data_dir, data_sign=data_sign, rel2idx=rel2idx)
            else:
