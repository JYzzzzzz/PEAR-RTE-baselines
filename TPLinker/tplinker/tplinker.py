...
class TPLinkerBert(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        head_rel_shaking_outputs = torch.stack(head_rel_shaking_outputs_list, dim=1)  # 24种关系拼接在一起
        tail_rel_shaking_outputs = torch.stack(tail_rel_shaking_outputs_list, dim=1)  # 24种关系拼接在一起
...
class MetricsCalculator():
    ...
    def get_sample_accuracy(self, pred, truth):
        ...
        pred_id = pred_id.view(pred_id.size()[0], -1)  # 关系时起到作用
    ...
    def get_rel_cpg(self, sample_list, tok2char_span_list,
        ...
        """
        返回一个batch中 正确的三元组个数，预测三元组个数，标签三元组个数
        """
        ...
        for ind in range(len(sample_list)):
            ...
            # case output
            # print(f"sample = \n  {sample}")
            # print(f"pred_rel_list = \n  {pred_rel_list}")    # 可作为实际案例输出
            sample['relation_list_pred'] = pred_rel_list
            sample_list_out.append(sample)

            # statistic
            gold_rel_list = sample["relation_list"]

            if pattern == "only_head_index":   # 判断主客体token span的起始位置数值
                gold_rel_set = set(["{}\u2E80{}\u2E80{}".format(
                    rel["subj_tok_span"][0],
                    rel["predicate"],
                    rel["obj_tok_span"][0])
            ...
            elif pattern == "whole_span":   # 判断主客体token span的完整位置数值
                gold_rel_set = set(["{}\u2E80{}\u2E80{}\u2E80{}\u2E80{}".format(
                    rel["subj_tok_span"][0],
                    rel["subj_tok_span"][1],
                    rel["predicate"],
            ...
            elif pattern == "whole_text":    # 判断主客体完整文本
            ...
            elif pattern == "only_head_text":    # 判断主客体由空格隔开的第一个文本，不适用与中文
        ...
        return (correct_num, pred_num, gold_num), sample_list_out
