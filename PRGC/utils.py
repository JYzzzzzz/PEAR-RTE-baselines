...
class Params:
    ...
    def __init__(self, args):  # ex_index=1, corpus_type='NYT', output_dir='experiments/240901'):
        self.root_path = Path(os.path.abspath(os.path.dirname(__file__)))
        # self.data_dir = self.root_path / f'data/{args.corpus_type}'
        self.data_dir = self.root_path / f'data/CMIM23-NOM1-RA'   # jyz chg
        # self.ex_dir = self.root_path / f'experiments/ex{ex_index}'
        self.ex_dir = self.root_path / f'{args.output_dir}'   # jyz chg
        self.model_dir = self.root_path / f'model/ex{args.ex_index}'
        # self.bert_model_dir = self.root_path / 'pretrain_models/bert_base_cased'
        self.bert_model_dir = self.root_path / 'pretrain_models/chinese-bert-wwm-ext'   # jyz chg

        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.n_gpu = torch.cuda.device_count()
        self.max_seq_length = 200  # 100    # jyz chg
        self.data_cache = False
        self.train_batch_size = args.batch_size    # 6 if 'WebNLG' in corpus_type else 64
        self.val_batch_size = 8   # 24
        self.test_batch_size = 8   # 64
        # PRST parameters
        ...
        # learning rate
        self.fin_tuning_lr = 1e-4 * args.lr_ratio
        self.downs_en_lr = 1e-3 * args.lr_ratio
        self.clip_grad = 2.    # 应该是梯度裁剪阈值
        self.drop_prob = args.dropout  # dropout
        self.weight_decay_rate = 0.01
        self.warmup_prop = 0.1
        self.gradient_accumulation_steps = args.grad_acc_steps    # 2 original
