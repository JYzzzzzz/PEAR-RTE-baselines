...
- 改用数据集文件中的 subj_char_span, obj_char_span 来获取实体位置。
- 修改了输出文件格式
- Any location with the annotation 'jyz chg'

## How to use after change

1. Check parameters in `main.py` and `script_run.sh`, especially the parameters related to *pre-trained model path* and *output path*.

2. Prepare the pre-trained model and place it in *pre-trained model path*.
