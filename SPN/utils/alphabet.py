...
class Alphabet:
    def __init__(self, name, padflag=True, unkflag=True, keep_growing=True):
                    # "Relation", unkflag=False, padflag=False
        self.name = name
        self.PAD = "</pad>"
        self.UNKNOWN = "</unk>"
        self.padflag = padflag
        self.unkflag = unkflag
        self.instance2index = {}    # !!!!!!!!!!!!!!!!!!!! relation text: relation id
        self.instances = []        # list of relation text
        self.keep_growing = keep_growing   # 一个是否允许更新 instance2index 的flag
        self.available_id = 0    # next available(idle) relation id
        self.index_num = {}    # relation id: 实体个数
        if self.padflag:
            self.add(self.PAD)
        if self.unkflag:
    ...
    def add(self, instance):    # 新增一个 关系-id 对
        if instance not in self.instance2index:
            self.instances.append(instance)
            self.instance2index[instance] = self.available_id
            self.index_num[self.available_id] = 1
            self.available_id += 1
    ...
    def get_index(self, instance):
        try:
            ...
            return index
        except KeyError:  # 未找到 “键”
            if self.keep_growing:
                index = self.available_id
                self.add(instance)
                return index
            else:
